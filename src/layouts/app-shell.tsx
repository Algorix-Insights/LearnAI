  ...
  return (
    <div className="min-h-screen">
      <div className="mx-auto grid min-h-[calc(100vh-2rem)] max-w-[1600px] overflow-hidden border border-[color:var(--app-border)] bg-[color:var(--app-surface)] shadow-[0_20px_80px_rgba(67,56,202,0.10)] backdrop-blur-xl lg:grid-cols-[280px_minmax(0,1fr)]">
        <aside className="flex flex-col border-b border-[color:var(--app-border)] bg-white/60 px-5 py-5 lg:border-b-0">
          ...
          <nav className="space-y-2">
            ...
            {navItems.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className={`flex items-center gap-3 rounded-2xl px-4 py-3 text-sm font-medium transition ${item.href === activeHref
        ...
        <section className="flex min-w-0 flex-col">
          <header className="flex flex-wrap items-center gap-4  bg-white/40 px-5 py-4 sm:px-6 lg:px-8">
